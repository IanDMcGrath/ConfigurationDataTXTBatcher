...
async function largeFileIntoBatch() {
  ...
  let writer = await fs.createWriteStream(batchOutput+batchName+'_'+batchNumber+'.txt','utf-8');
...
getFirstAndLastLine = (fileName, minLength) => {
...
async function inspectOutput() {
  ...
  for await (const filename of files) {
    if (filename.slice(filename.length-5,filename.length) === '.keep') {
      continue;
    }
    const lines = await getFirstAndLastLine('output/'+filename,0);
    const numStartColumns = lines[0].split('|').length;
    const numEndColumns = lines[1].split('|').length;
    const err = numStartColumns !== numEndColumns? 'FAILURE' :'';
    console.log(filename,numStartColumns,numEndColumns,err);
    // console.log(lines);
